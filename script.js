...
class Order{
    ...
    exportPayment(date){
        const currrentPayment = [];
        const tipChange = Utilities.roundToTwo(this.payment.amountPaid-this.getSummary().grandtotal);
        currrentPayment[0] =date;
        currrentPayment[1] = this.invoiceNumber;

        currrentPayment[2] = this.getSummary().grandtotal;
        currrentPayment[3] = this.payment.type;
        if(this.payment.type == 'cash'){
            currrentPayment[4] = 0;
        }
        else{
            currrentPayment[4] = tipChange;
        }
        paymentsData.push(currrentPayment);
    }
    closeSale(){
        const date = new Date();
...
class Ui{
    static menu(orderInstance)
    {
        ...
        orderInstance.menu.forEach(item=>{
            let menuElement = `<img src="${item.image}" alt="" class="menu-img" style="width: 150px;">
    ...
    static receiptDetails(orderInstance){
        ...
        orderInstance.order.forEach((orderLine,index)=>{
            let receiptLine = ` <td class="description">${orderLine.description}</td>
                                ...
                                <td class="delete-btn" data-delete="${index.toString()}"><i class="fas fa-backspace"></i></td>`
        ...
        this.summaryUI(orderInstance);
        document.querySelectorAll(".delete-btn").forEach(td=>{
            td.addEventListener("click",()=>{
                orderInstance.deleteOrderLine(parseInt(td.getAttribute("data-delete")));
            })
        })

    }
    static invoiceNumberUi(orderInstance){
    ...
    static summaryUI(orderInstance){
        const sumarry = orderInstance.getSummary();
        ...
        subTotal.textContent = Utilities.convertFloatToString(sumarry.subtotal);
        tax.textContent = Utilities.convertFloatToString(sumarry.tax);
        grandtotal.textContent = Utilities.convertFloatToString(sumarry.grandtotal);
...
class Utilities{
     ...
     static backPaypad(orderInstance){
        const currrentPayment = orderInstance.payment.amountPaid;
        if(currrentPayment>0){
            const toSubtract = ((currrentPayment*100)%10)*0.01;
            const newAmount = (currrentPayment-toSubtract)*0.1;
